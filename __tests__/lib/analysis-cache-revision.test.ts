...
import { cvValueBar, neverInventRule, proseRules } from "@/lib/services/ai/shared/cv-writing-doctrine"
...
describe("la revision del analisis sigue a la doctrina", () => {
  ...
  it("la huella cubre las tres piezas que el prompt inyecta", () => {
    ...
    expect(MOD).toMatch(/neverInventRule\("es"\)/)
  ...
  it("una doctrina distinta produce una huella distinta", () => {
    const fp = (extra: string) =>
      createHash("sha256")
        .update(cvValueBar("es") + extra + neverInventRule("es") + proseRules("es"))
        .digest("hex")
        .slice(0, 8)
  ...
  it("la doctrina vigente autoriza el rango confirmable", () => {
    expect(neverInventRule("es")).toMatch(/RANGO/)
    expect(neverInventRule("es")).not.toMatch(/escribi la linea sin numero/i)
  })
})

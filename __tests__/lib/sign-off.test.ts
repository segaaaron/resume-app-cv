...
import { stripSignOff, detectHallucination } from "@/lib/services/ai/shared/ai-helpers"
...
describe("detectHallucination — metric placeholders only", () => {
  ...
  it("does not flag a name or company placeholder as a metric", () => {
    expect(detectHallucination("Sincerely, [Your Name]", source)).toBe(false)
    expect(detectHallucination("I want to join [Company].", source)).toBe(false)
  })

  it("still flags real metric placeholders", () => {
    expect(detectHallucination("Grew revenue by [X%].", source)).toBe(true)
    expect(detectHallucination("Served [N users] daily.", source)).toBe(true)
    expect(detectHallucination("Saved [$Z] per quarter.", source)).toBe(true)
    expect(detectHallucination("Shipped in [3 months].", source)).toBe(true)
    expect(detectHallucination("Reached [number of clients].", source)).toBe(true)
  })

  it("still flags a metric absent from the source", () => {
    expect(detectHallucination("Cut crashes by 40% last year.", source)).toBe(true)
  })

  it("passes clean prose anchored to the source", () => {
    expect(detectHallucination("I ship iOS apps at Xiobit and help my team deliver features.", source)).toBe(false)
  })
})

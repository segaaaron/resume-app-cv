...
import { getCronHealth } from "@/lib/services/cron/cronHealth"
import CronHealthPanel, { type CronHealthView } from "@/components/admin/CronHealthPanel"

export const dynamic = "force-dynamic"

export const metadata = { title: "Admin — Cron Health", robots: { index: false, follow: false } }

export default async function AdminCronsPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: "dashboard_admin.crons" })
  ...
  const jobs: CronHealthView[] = health.map((h) => ({
    job: h.job,
    label: h.meta.label,
    scheduleHuman: h.meta.scheduleHuman,
    state: h.state,

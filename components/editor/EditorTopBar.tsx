...
import { ArrowLeft, Save, Download, Loader2, Check, Lock, History, RotateCcw, Trash2 } from "lucide-react"
...
export default function EditorTopBar({ hasAccess }: Props) {
  ...
  async function handleRestore(versionId: string) {
    ...
    if (!res.ok) { toast.error("Error al restaurar versión"); return }
      ...
      {showHistory && (
        <div className="absolute top-12 right-4 z-50 w-80 bg-white border border-border rounded-xl shadow-lg overflow-hidden">
          ...
          <div className="max-h-72 overflow-y-auto divide-y divide-border">
            {loadingVersions ? (
              ...
              <div className="py-8 text-center text-sm text-muted-foreground px-4">
                ...
                Sin versiones guardadas.<br />
                <span className="text-xs">Haz clic en "Guardar versión" para crear una.</span>
              </div>
            ) : (
              versions.map((v) => (
                <div key={v.id} className="flex items-center justify-between px-4 py-2.5 hover:bg-muted/40 transition-colors">
                  <div>
                    <p className="text-xs font-medium">{v.label ?? "Sin nombre"}</p>
          ...
          <div className="px-4 py-2 border-t border-border">
            <p className="text-[11px] text-muted-foreground text-center">Máximo 10 versiones · Las más antiguas se eliminan automáticamente</p>
          </div>
        </div>
      )}
    </header>

...
describe("withPage", () => {
  it("resolves with fn return value", async () => {
    expect(await withPage(async () => "ok")).toBe("ok")
  })
  ...
  it("calls page.close() even when fn throws", async () => {
    await expect(withPage(async () => { throw new Error("boom") })).rejects.toThrow("boom")
    expect(mockClose).toHaveBeenCalledTimes(1)
  })
...
describe("concurrency — queue path", () => {
  it("queues second withPage when slot is full and processes it after first completes", async () => {
    ...
    let signalSlotAcquired!: () => void
    const slotAcquired = new Promise<void>((r) => { signalSlotAcquired = r })
    let releaseFirst!: () => void
    const blocker = new Promise<void>((r) => { releaseFirst = r })
    ...
    const p1 = withPage(async () => { signalSlotAcquired(); await blocker; return "first" })

    // Wait until p1 is definitely executing (slot acquired)
    await slotAcquired

    const p2 = withPage(async () => "second")

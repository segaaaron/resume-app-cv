...
function browserFixLayout(pagePx: number, fudgePx: number, bottomMarginPx: number, resumePagesSelector: string, resumeHeadingSelector: string): void {
  ...
  adjustLayout(root, wrapper, zoom, eff, fudgePx)
  fixHeadingOffsets(wrapper, wrapper.getBoundingClientRect().top, eff, resumeHeadingSelector)
}

function isDiscretePages(children: HTMLElement[], pagePx: number, zoom: number): boolean {
  return children.length > 1 && children.every((c) => Math.abs(c.scrollHeight * zoom - pagePx) < pagePx * 0.15)
}

function hideEmptyLastPage(children: HTMLElement[], pagePx: number, zoom: number, fudgePx: number): void {
  const last = children[children.length - 1]
  if ((last.scrollHeight * zoom - fudgePx) / pagePx < 0.15) last.style.setProperty("display", "none", "important")
}
...
function buildGradientCss(side: "left" | "right", ratio: number, sidebarBg: string, mainBg: string): string {
  if (side === "left") return `linear-gradient(to right, ${sidebarBg} 0%, ${sidebarBg} ${ratio}%, ${mainBg} ${ratio}%, ${mainBg} 100%)`
  return `linear-gradient(to left, ${sidebarBg} 0%, ${sidebarBg} ${100 - ratio}%, ${mainBg} ${100 - ratio}%, ${mainBg} 100%)`
}
...
function paintSidebarGradient(root: HTMLElement): void {
  ...
  root.style.setProperty("background", buildGradientCss(side, ratio, sidebarBg, mainBg), "important")
}

function adjustLayout(root: HTMLElement, wrapper: HTMLElement, zoom: number, eff: number, fudgePx: number): void {
...
function getColumnsWithPadding(root: HTMLElement): { el: HTMLElement; paddingPx: number }[] {
  const { sidebarEl, mainEl } = getSidebarConfig(root)
  const fallback = root
  const cols = [(mainEl ?? fallback), sidebarEl].filter(Boolean) as HTMLElement[]
  return cols.map((el) => ({ el, paddingPx: parseFloat(window.getComputedStyle(el).paddingTop) || 0 })).filter((c) => c.paddingPx > 0)
}

function injectColumnSpacers(root: HTMLElement, wrapperTop: number, eff: number, numPages: number): void {
  getColumnsWithPadding(root).forEach(({ el, paddingPx }) => doInjectSpacers(el, wrapperTop, eff, numPages, paddingPx))
}

function doInjectSpacers(col: HTMLElement, wrapperTop: number, eff: number, numPages: number, paddingPx: number): void {
  const colRect = col.getBoundingClientRect()
  const cx = colRect.left + colRect.width / 2
  col.querySelectorAll("[data-pdf-spacer]").forEach((s) => s.remove())
  for (let pN = numPages - 1; pN >= 1; pN--) insertSpacerAt(col, cx, wrapperTop + pN * eff, paddingPx)
}
...
function snapRootHeight(root: HTMLElement, contentBottomPx: number, eff: number, zoom: number): void {
  const numPages = Math.ceil(contentBottomPx / eff)
  if (numPages <= 1) return
  const lastFill = (contentBottomPx - (numPages - 1) * eff) / eff
  const h = lastFill < 0.05 ? ((numPages - 1) * eff) / zoom : Math.max((numPages * eff) / zoom, root.scrollHeight)
  root.style.setProperty("height", `${h}px`, "important")
  root.style.setProperty("min-height", lastFill < 0.05 ? "0" : `${h}px`, "important")
  root.style.setProperty("overflow", "hidden", "important")
}

function fixHeadingOffsets(wrapper: HTMLElement, wrapperTop: number, eff: number, headingSelector: string): void {
  Array.from(wrapper.querySelectorAll<HTMLElement>(headingSelector))
    .filter((el) => !(el as HTMLElement & { dataset: DOMStringMap }).dataset.pdfSpacer)
    .forEach((el) => {
      const off = (el.getBoundingClientRect().top - wrapperTop) % eff

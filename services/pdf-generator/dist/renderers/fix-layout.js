...
async function fixLayout(page) {
    const grad = await page.evaluate(browserFixLayout, constants_1.USABLE_PX_PER_PAGE, constants_1.FUDGE_PX, constants_1.PDF_BOTTOM_MARGIN_PX, contracts_1.RESUME_PAGES_SELECTOR, contracts_1.RESUME_HEADING_SELECTOR);
    if (grad) {
 ...
 */
function browserFixLayout(pagePx, fudgePx, bottomMarginPx, resumePagesSelector, resumeHeadingSelector) {
    function isDiscretePages(children, ppx, z) {
        return children.length > 1 && children.every((c) => Math.abs(c.scrollHeight * z - ppx) < ppx * 0.15);
    }
    function hideEmptyLastPage(children, ppx, z, fpx) {
        const last = children[children.length - 1];
        if ((last.scrollHeight * z - fpx) / ppx < 0.15)
            last.style.setProperty("display", "none", "important");
    }
    function getSidebarConfig(root) {
        const layout = root.dataset.printLayout ?? "";
        if (layout === "sidebar-left")
            return { sidebarEl: root.firstElementChild, mainEl: root.lastElementChild, side: "left" };
        if (layout === "sidebar-right")
            return { sidebarEl: root.lastElementChild, mainEl: root.firstElementChild, side: "right" };
        return { sidebarEl: null, mainEl: null, side: null };
    }
    function getSolidBg(el) {
        const ok = (bg) => bg && bg !== "rgba(0, 0, 0, 0)" && bg !== "transparent";
    ...
    function paintSidebarGradient(root) {
        const { sidebarEl, mainEl, side } = getSidebarConfig(root);
        if (!sidebarEl || !mainEl || !side)
            return null;
    ...
    function snapRootHeight(root, contentBottomPx, eff, z) {
    ...
    const wrapper = document.querySelector(resumePagesSelector);
    if (!wrapper)
        return null;
    ...
    if (isDiscretePages(children, pagePx, zoom)) {
        hideEmptyLastPage(children, pagePx, zoom, fudgePx);
        return null;
    }
    ...
    Array.from(wrapper.querySelectorAll(resumeHeadingSelector))
        .filter((el) => !el.dataset.pdfSpacer)
        .forEach((el) => {

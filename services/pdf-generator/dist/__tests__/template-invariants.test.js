...
const TEMPLATES_DIR = path_1.default.resolve(__dirname, "../../../../components/resume/templates");
...
describe("Resume template invariants", () => {
    ...
    it("every template sets data-print-layout on its root div", () => {
        const missing = templateFiles.filter((file) => {
            const content = fs_1.default.readFileSync(path_1.default.join(TEMPLATES_DIR, file), "utf8");
            return !DATA_PRINT_LAYOUT_RE.test(content);
        });
        if (missing.length > 0) {
            throw new Error(`${missing.length} template(s) missing data-print-layout attribute.\n` +

...
> **REGLA DE ORO — Sin excepción:** Ningún agente hace `git commit`, `git push`, ni crea PR sin autorización explícita del usuario en ese turno. Preguntar siempre antes. Un "sí" anterior no cuenta.

## Infraestructura — Contexto permanente

- **Deploy:** Dokploy en servidor Hostinger
- **Rama de trabajo:** siempre la rama activa en el momento (`git branch --show-current`). NUNCA comparar contra `master` ni verificar su estado — `master` lo controla el usuario.
- **Regla:** trabajar solo con lo que existe en la rama actual. Sin asumir estado de `master`.

This version has breaking changes — APIs, conventions, and file structure may all differ from your training data. Read the relevant guide in `node_modules/next/dist/docs/` before writing any code. Heed deprecation notices.
<!-- END:nextjs-agent-rules -->

## Regla de QA — Sin excepción

**Toda feature nueva o fix debe pasar por el agente `readycvv-qa-senior` antes de hacer commit/deploy.** El agente da el visto bueno. Sin eso, no se sube nada.
...
Todos configurados y **activos en Dokploy**. Todos protegidos con `Authorization: Bearer <CRON_SECRET>`.

| Nombre en Dokploy | Endpoint | Cron | Qué hace |
|-------------------|----------|------|----------|
| Renewal Reminder | `GET /api/cron/renewal-reminder` | `0 9 * * *` | Email recordatorio de renovación a usuarios Pro próximos a expirar |
| Application Reminders | `GET /api/cron/application-reminders` | `0 8 * * *` | Email recordatorio de candidaturas pendientes en kanban |
| Purge Stripe Events | `GET /api/cron/purge-stripe-events` | `0 3 * * 0` | Borra registros `StripeEvent` con más de 90 días (idempotencia) |
| Expire Subscriptions | `GET /api/cron/expire-subscriptions` | `0 2 * * *` | Baja usuarios Pro cuyo `subscriptionEndsAt` pasó (CANCELED + drift guard ACTIVE). Purga caché de sesión. |
| GDPR Data Cleanup | `GET /api/cron/data-cleanup` | `0 2 * * *` | Elimina cuentas con `deletedAt` > 90 días (GDPR Art. 17) + purga `AIUsageLog` > 90d |
...
**Pendientes de seguridad:** ver `memory/security_audit.md` — 2 LOW + 2 INFO sin resolver. 18 fixes implementados en 2026-04-29.
...
### claude-mem (Skills — invocar con Skill tool)

| Cuándo | Skill |
|--------|-------|
| Inicio de sesión nueva, orientarse rápido | `claude-mem:smart-explore` |
| Buscar decisión o patrón de sesión anterior | `claude-mem:mem-search` |

...
- termVariants: for each hardSkill, the wordings it would appear as IN A RESUME for that trade. A posting names the requirement as a noun — "Reconciliation of receipts" — and a resume writes what the person DID, in the past tense: "reconciled receipts", "performed daily cash counts", "handled the reconciliation of receipts". IF YOU DO NOT GIVE THE VERB FORM, THE CANDIDATE GETS NO CREDIT FOR SOMETHING THEY ALREADY WROTE — that is the case that matters most, so the first variant of every term must be the verb form.
  Beyond the verb form: the everyday name for it, the abbreviation, the tool or standard it is known by in that trade. It comes from the trade this posting is about, never from a general list. At most 4 per term.
...
- termVariants: por cada hardSkill, las formas en que aparecería en un CV DE ESE OFICIO. Un aviso nombra el requisito como sustantivo —«Conciliación de comprobantes»— y un CV lo escribe como lo que la persona HIZO, en primera persona y en pasado: «concilié comprobantes», «realicé la conciliación de comprobantes», «arqueo diario de caja». SI NO DAS LA FORMA VERBAL, EL CANDIDATO NO COBRA POR ALGO QUE YA ESCRIBIÓ — es el caso que más importa, así que la primera variante de cada término tiene que ser la verbal.
  Además de la verbal: la manera cotidiana de nombrarlo, la sigla, la herramienta o la norma con que se lo conoce en ese oficio. Sale del oficio del que habla ESTE aviso, nunca de una lista general. Como máximo 4 por término.
